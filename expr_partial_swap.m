function expr_partial_swap(compile, model)
...
%     new_box_y_pos = randi([boxes(2) boxes(4)-box_y_size]);
    swapfrom = im(box_y_pos:box_y_pos+box_y_size, box_x_pos:box_x_pos+box_x_size, :);
    swapto = im(new_box_y_pos:new_box_y_pos+box_y_size, new_box_x_pos:new_box_x_pos+box_x_size, :);
    im(box_y_pos:box_y_pos+box_y_size, box_x_pos:box_x_pos+box_x_size, :) = flipud(fliplr(swapto));
    im(new_box_y_pos:new_box_y_pos+box_y_size, new_box_x_pos:new_box_x_pos+box_x_size, :) = swapfrom;
    this_box = test(im, model, 1, ground_truth);
    iou(i) = calculate_iou(this_box(1:4), ground_truth);
end
% plot IoU
figure
plot(iou)
